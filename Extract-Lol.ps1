...
function Seek($inFile, [Int32] $bufSize){
  ...
  while( $bytesRead = $stream.Read($barr,0,$bufSize)){
    ...
    $Regex = [regex] '\x6C\x6F\x6C\x63\x6C\x69\x65\x6E\x74\x2E\x6C\x6F\x6C\x2E\x72\x69\x6F\x74\x67\x61\x6D\x65\x73\x2E\x63\x6F\x6D'
    $Results = $Regex.Matches($RawString)
	if ($Results.count -gt 0)
	{
        $len = $Results.count
        for($i=0;$i-lt$len;$i++){
            $match = $RawString.substring($Results[$i].Index - 200, 600)
